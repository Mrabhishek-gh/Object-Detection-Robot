...
#define LEFT A5               // Left IR sensor connected to analog pin A3 of Arduino Uno:
...
void forward()
{
  Motor1.setSpeed(speedf);  //define motor1 speed:
  Motor1.run(FORWARD);   //rotate motor1 clockwise:
  Motor2.setSpeed(speedf);  //define motor2 speed:
  Motor2.run(FORWARD);   //rotate motor2 clockwise:
  Motor3.setSpeed(speedf);  //define motor3 speed:
  Motor3.run(FORWARD);   //rotate motor3 clockwise:
  Motor4.setSpeed(speedf);  //define motor4 speed:
  Motor4.run(FORWARD);   //rotate motor4 clockwise:
}    

void turnRight()
{
  Motor1.setSpeed(speedr);  //define motor1 speed:
  Motor1.run(BACKWARD);  //rotate motor1 anticlockwise:
  Motor2.setSpeed(speedr);  //define motor2 speed:
  Motor2.run(BACKWARD);  //rotate motor2 anticlockwise:
  Motor3.setSpeed(speedr);  //define motor3 speed:
  Motor3.run(FORWARD);   //rotate motor3 clockwise:
  Motor4.setSpeed(speedr);  //define motor4 speed:
  Motor4.run(FORWARD);   //rotate motor4 clockwise:
  delay(1500);
}    
...
void turnLeft()
{
  Motor1.setSpeed(speedl);  //define motor1 speed:
  Motor1.run(FORWARD);   //rotate motor1 cloclwise:
  Motor2.setSpeed(speedl);  //define motor2 speed:
  Motor2.run(FORWARD);   //rotate motor2 clockwise:
  Motor3.setSpeed(speedl);  //define motor3 speed:
  Motor3.run(BACKWARD);  //rotate motor3 anticlockwise:
  Motor4.setSpeed(speedl);  //define motor4 speed:
...
void backward()
{
  Motor1.setSpeed(speedb);  //define motor1 speed:
  Motor1.run(BACKWARD);   //rotate motor1 anticlockwise:
  Motor2.setSpeed(speedb);  //define motor2 speed:
  Motor2.run(BACKWARD);   //rotate motor2 anticlockwise:
  Motor3.setSpeed(speedb);  //define motor3 speed:
  Motor3.run(BACKWARD);   //rotate motor3 anticlockwise:
  Motor4.setSpeed(speedb);  //define motor4 speed:
  Motor4.run(BACKWARD);   //rotate motor4 anticlockwise:
...
if((distance > 5) && (distance < 15)){            //check wheather the ultrasonic sensor's value stays between 1 to 15.                                              //If the condition is 'true' then the statement below will execute:
  forward();
}
